...
public class StageB {
	...
	// runtime to set the maximum queue
	...
	// reusable menu to run method easy reintegration
	...
	// removes job front of queue and reloads menu
	private static void removePrintJob() {
		PrintQueue.removeJob();
		menu();
	}

	// search the job by print Id to display all details including pricing
	private static void searchPrintJob() {
		...
		while (!exists) {
			...
			System.out.println("");
			// fancy table for display
			System.out.format(
					"+-----------+----------------+----------------+----------------+-----------+-----------+-------------+-------------+%n");
			System.out.format(
					"| Print ID  |  Customer ID   |   Description  |   Material     | Quantity  |   Volume  |  Raw Price  | Total Price |%n");
			System.out.format(
					"+-----------+----------------+----------------+----------------+-----------+-----------+-------------+-------------+%n");
	...
	// add job will sent to printjob or Processed depending on entries
	private static void addPrintJob() {
		if (currentIndex < maxPrintJobs) {
			...
			if (tempInput == 1 || matSelect == 4) {
				...
				// checks if atleast 1 entry is true as processing was selected restart menu if
				// is not for simple quality control
				if (!processing1 && !processing2 && !processing3) {
					System.out.println("YOU NEED TO SELECT 1 NOW YOU CAN RE-ENTER FROM START!!!!!!\n");
					menu();
				} else {
					PrintQueue.addProcessed(printID, custID, description, material, printQuantity, volume, processing1,
	...
	// initialize counters and setup of program
